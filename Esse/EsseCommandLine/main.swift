...
struct Esse: ParsableCommand {
    ...
    func runFunctions(text: String, functions: [TextFunction]) -> String {
        let actions = functions.compactMap(\.actions).flatMap { $0 }
...
extension Esse {
    struct List: ParsableCommand, Decodable {
        ...
        mutating func run() {
            ...
            if alfred != 0 {
                let alfredItems = functions.map(\.alfred)
...
extension TextFunction {
    ...
    var alfred: AlfredItem {
        AlfredItem(uid: id, title: title, subtitle: desc, match: "\(title) \(desc)", arg: id, autocomplete: title)
    }
}
